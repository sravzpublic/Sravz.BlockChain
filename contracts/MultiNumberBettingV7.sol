...
contract MultiNumberBettingV7 is MultiNumberBettingAbstractV3 { 
  ...
  event NewHighBid(address indexed winnerAddress, string winnerName, uint amount);
  event BidFailed(address indexed loserAddress, string loserName, uint amount);
  
 function MultiNumberBettingV7(uint num1, uint num2, uint num3) {
    if ((num1 >= 1 && num1 <=10 ) && 
    	(num2 >= 1 && num2 <=10) && 
    	(num3 >= 1 && num3 <=10))
    {
